...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      of: [{
        type: 'object',
        fields: [
          { name: 'name', title: 'Variation Name', type: 'string', validation: (Rule) => Rule.required() },
  ...
  preview: {
    select: { title: 'title', media: 'images.0', price: 'basePrice', active: 'active' },
    prepare({ title, media, price, active }) {
      return { title: `${active ? '' : '🚫 '}${title}`, subtitle: `$${price?.toFixed(2) || '0.00'}`, media };
    },
  },
});
